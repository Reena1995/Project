	...
	function __construct()
	{
		...
		switch($path)
		{
			...
			if(isset($_REQUEST['submit']))
			{
				...
				{
					echo "<script>
					alert('login failed due to client is unverified');
			...
			if(isset($_REQUEST['submit']))
			{
				...
				$data=array("shop_name"=>$shop_name,"address"=>$address,"loc_id"=>$loc_id,"about_shop"=>$about_shop,"Ownername"=>$Ownername,
				"email"=>$email,"mobile_no"=>$mobile_no,"client_id"=>$client_id,"img"=>$img,"created_dt"=>$curent_dt,
				"updated_dt"=>$curent_dt);
			...
			$res=$this->select_where_join3('shop','location','shop.loc_id=location.loc_id','client','shop.client_id=client.client_id',$where);
			...
			if(isset($_REQUEST['btn_shop_id']))
			{
				...
				if(isset($_REQUEST['update']))
				{
					...
					{
						//img upload
						$car_img=$_FILES['img']['name'];
						$path='../upload/shop/'.$img;
						$dup_img=$_FILES['img']['tmp_name'];
						move_uploaded_file($dup_img,$path);
		
						$data=array("shop_name"=>$shop_name,"address"=>$address,"loc_id"=>$loc_id,"about_shop"=>$about_shop,"Ownername"=>$Ownername,"email"=>$email,"mobile_no"=>$mobile_no,"img"=>$img,"Updated_dt"=>$Updated_dt);
						$res=$this->update_where('shop',$data,$where);
						if($res)
						{
							unlink('../upload/shop/'.$old_img);
							echo "<script>
					...
					{
						$data=array("shop_name"=>$shop_name,"address"=>$address,"loc_id"=>$loc_id,"about_shop"=>$about_shop,"Ownername"=>$Ownername,"email"=>$email,"mobile_no"=>$mobile_no,"Updated_dt"=>$Updated_dt);
						$res=$this->update_where('shop',$data,$where);
						if($res)
						{
							echo "<script>
